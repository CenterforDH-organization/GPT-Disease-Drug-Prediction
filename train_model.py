...
# Composite Delphi model config (6-column data)
...
if model_type == 'composite':
    ...
    train_data = np.memmap(os.path.join(data_dir, 'train.bin'), dtype=composite_dtype, mode='r')
    val_data = np.memmap(os.path.join(data_dir, 'val.bin'), dtype=composite_dtype, mode='r')
...
if model_type == 'composite':
    ...
    model_args = dict(
        ...
        loss_weight_unit=loss_weight_unit,
    ...
    if init_from == 'scratch':
        ...
        for k in ['n_layer', 'n_head', 'n_kv_head', 'n_embd', 'block_size', 'bias',
                  'data_vocab_size', 'dose_vocab_size', 'total_vocab_size', 'unit_vocab_size']:
...
def estimate_loss_composite():
    """Estimate loss for Composite Delphi (6-column data)"""
    out = {}
    model.eval()
    for split in ['train', 'val']:
        losses = torch.zeros(eval_iters, 6)  # loss, data, dose, total, unit, time
        data = train_data if split == 'train' else val_data
        p2i = train_p2i if split == 'train' else val_p2i
        for k in range(eval_iters):
            ix = torch.randint(len(p2i), (batch_size,))
            batch = get_batch_composite(ix, data, p2i, block_size=block_size,
                                        ...
                                        no_event_token_rate=no_event_token_rate,
                                        cut_batch=True)
            x_data, x_dose, x_total, x_unit, x_ages, y_data, y_dose, y_total, y_unit, y_ages = batch
...
while True:
    ...
    if iter_num % eval_interval == 0 and iter_num > 0:
        ...
        if model_type == 'composite':
            # Composite model has 6 loss components
            if val_loss is None:
            ...
            print(f"  breakdown - data: {val_loss_unpooled[1].item():.4f}, dose: {val_loss_unpooled[2].item():.4f}, "
                  f"total: {val_loss_unpooled[3].item():.4f}, unit: {val_loss_unpooled[4].item():.4f}, time: {val_loss_unpooled[5].item():.4f}")
            ...
            if wandb_log:
                wandb.log({
                    ...
                    "val/loss_unit": val_loss_unpooled[4].item(),
        ...
        if wandb_log:
            ...
            if model_type == 'composite':
                log_dict.update({
                    ...
                    "train/loss_unit": loss['loss_unit'].item(),
