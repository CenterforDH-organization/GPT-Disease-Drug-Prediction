...
# wandb logging
wandb_log = True
wandb_project = 'composite-delphi-shift'
...
dropout = 0.3
bias = False
vocab_size = 1290  # Must include Death token (raw 1288 â†’ shifted 1289)

# Composite Delphi model config (5-column data)
data_vocab_size = 1290   # DATA: ì•½í’ˆ/ì§ˆë³‘ ì½”ë“œ ìˆ˜ (Classification)
shift_vocab_size = 5     # SHIFT: Classification (values 0-4)
total_vocab_size = 552   # TOTAL: Embedding vocab

# SHIFT imbalance handling
shift_loss_type = 'dice_focal'      # CHANGED: 'dice_focal' (NEW), 'focal', or 'ce'
shift_ignore_index = 0
shift_focal_gamma = 2.0
shift_class_weights = []
shift_dice_weight = 0.5             # NEW: Dice vs Focal blend ratio
shift_maintain_idx = 2              # NEW: which SHIFT class index is "Maintain"

# Loss weights for composite model
loss_weight_data = 1.0
loss_weight_shift = 15.0
loss_weight_total = 1.0   # CHANGED: MDN NLL is ~2-5, not ~0.001 like MSE/scale^2
loss_weight_time = 0.1
loss_weight_change = 2.0            # NEW: binary change detection weight

# MDN parameters for TOTAL regression
mdn_n_components = 8                # NEW: number of mixture components
...
if model_type == 'composite':
    ...
    # Define drug token range (used by both class weight computation and patient sampling)
    ...
    for pid, (start_idx, length) in enumerate(train_p2i):
        ...
        patient_weights[pid] = 1.0 + minority_count * 0.3     # minority_count 1.0 -> 0.3
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            ...
            losses[k] = torch.stack([
                ...
                loss['loss_data'],
                loss['loss_shift'],
                loss.get('loss_change', torch.tensor(0.0)),
                loss['loss_total'],
                loss['loss_time']
            ])
        out[split] = losses.mean(0)
...
while True:
    ...
    if iter_num % 1000 == 0 and model_type == 'composite':
        ...
        for name, param in model.named_parameters():
            if param.grad is not None:
                if any(k in name for k in ['total_head', 'time_head', 'time_shape_head',
                                             'change_detector', 'params_head']):
                    grad_norm = param.grad.norm().item()
                    short_name = name.split('.')[-2] + '.' + name.split('.')[-1]
                    grad_info.append(f"{short_name}={grad_norm:.6f}")
        if grad_info:
            print(f"  [GRAD DEBUG] {', '.join(grad_info[:8])}")
    ...
    if iter_num % log_interval == 0:
        ...
        if iter_num > 0 and iter_num % (log_interval * 10) == 0:
            ...
            lr_change = "â†‘" if lr > prev_lr else "â†“" if lr < prev_lr else "="
        ...
        if wandb_log:
            ...
            if model_type == 'composite':
                log_dict.update({
                    ...
                    "train/loss_change": loss.get('loss_change', torch.tensor(0.0)).item(),
